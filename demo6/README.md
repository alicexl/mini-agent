...
- **触发时机**：Plan 每个 step 开头调一次 `compact_messages`，**顶层决策前也调一次**（首条消息不会触发，但保持调用习惯一致）。4-step 任务实测触发 **3–4 次**压缩（讲稿 §5.2）
...
```
在 demo6 目录下创建一个 demo.txt 写入"hello demo6"，再读取它，最后告诉我文件大小
```

> 讲稿 §5 的真实示例任务用的是 `hello.txt`（产物已保留在仓库里），自己跑时换个文件名避免覆盖。
