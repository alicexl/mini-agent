...
> 在 demo1-react（base）上独立叠加「规划轴」：新增 `plan` 工具（自动决策版） + 接入 Skill 预消化工作流。

## 文档导航

- **[`讲稿.md`](讲稿.md)** — 完整教学讲稿（5 章）
  1. demo4 干了什么 & 为什么需要规划
  2. 机制一：plan（自动决策版）
  3. 机制二：Skill（预消化的工作流）
  4. 真实演示案例（简单任务跳过 plan / 复杂任务 5 步推进 / review skill 命中）
  5. 总结
...
| 文件 | 说明 |
|---|---|
| `agent.py` | Agent 主程序（Part 1-5：客户端 / 工具定义（含 plan）/ 工具实现 / Skill 加载器 / 主循环） |
...
- demo4 新增 `plan`——把多步任务的 step 列表显式化（对应 Claude Code 的 TodoWrite）
...
- **复杂任务示例**：`第一步读 agent.py；第二步提取函数名；第三步...严格按 N 步执行` —— LLM 先 plan 列步骤再逐步推进
...
- `todos.md` —— plan 工具的落盘文件（每次调用覆盖式更新），已加入 `.gitignore`
